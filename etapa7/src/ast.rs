...
use crate::{
    ...
    iloc_aux::{
        save_rfp_rsp, CmpInst, FullOp, IlocInst, In2Out, InOut, Inv, Jump, RETVAL_ADDR, RET_ADDR,
        RFP_ADDR, RSP_ADDR,
    },
};
...
impl ASTNode {
    ...
    pub fn temp(&self) -> String {
        match self {
            ...
            ASTNode::ExprInv(node) => node.temp.clone(),
...
impl InvSigOp {
    pub fn new(span: Span, child: Box<ASTNode>) -> Self {
        ...
        #[cfg(feature = "code")]
        let code = {
            let not = IlocInst::Inv(Inv::new(child.temp(), temp.clone()));
